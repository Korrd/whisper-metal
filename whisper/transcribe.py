...
def transcribe(
    ...
    **decode_options,
    ...
    if model.device == torch.device("cpu"):
        # Workaround for MPS sparse tensor crash
        model = model.to("cpu")
        model.encoder.to("mps")
        model.decoder.to("mps")
        device = "mps"  # continue using MPS for inference
...
def cli():
    ...
    parser.add_argument("--device", default="cuda" if torch.cuda.is_available() else "cpu", choices=["cuda", "cpu", "mps_safe"], help="device to use for PyTorch inference")
